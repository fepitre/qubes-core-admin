...
class QubesAdminAPI(qubes.api.AbstractQubesAPI):
    ...
    async def vm_volume_list(self):
        ...
        volume_names = self.fire_event_for_filter(self.dest.volumes.keys()) if isinstance(self.dest, qubes.vm.qubesvm.QubesVM) else []
    ...
    async def vm_remove(self):
        ...
        async with self.dest.startup_lock:
            ...
            if isinstance(self.dest, qubes.vm.qubesvm.QubesVM) and self.dest.installed_by_rpm:
                raise qubes.exc.QubesVMInUseError(
                    self.dest,
    ...
    async def vm_device_list(self, endpoint):
        ...
        dev_info = {
            f"{assignment.backend_domain}+{assignment.ident}": assignment.serialize().decode(
                "ascii", errors="ignore"
            )
    ...
    # For this reason, write=True
    @qubes.api.method(
        "admin.vm.device.{endpoint}.Set.required",
        endpoints=(
            ep.name
            for ep in importlib.metadata.entry_points(group="qubes.devices")
        ),
